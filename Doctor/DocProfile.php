...
if (isset($_POST['update_profile'])) {
    ...
    if (empty($full_name) || empty($email) || empty($phone) || empty($specialist) || empty($city) || empty($new_username)) {
        ...
        if ($new_username != $current_username) {
            $check_username = mysqli_query($conn, "SELECT * FROM users WHERE username = '$new_username'");
        ...
        if (empty($error_message)) {
            ...
            if (mysqli_query($conn, $update_doctor)) {
                $update_user = "UPDATE users SET username = '$new_username'";
...
<head>
    ...
    <style>
        ...
        .success {background:#d4edda;color:#155724;padding:10px;text-align:center;margin-bottom:15px;}
        .error {background:#f8d7da;color:#721c24;padding:10px;text-align:center;margin-bottom:15px;}
        <style>
    .appointment .form {
        display: flex;
        flex-direction: column;
    ...
    input[type="file"].box {
        padding: 8px;          /* Better file input spacing */
    }
    .current-photo {
        text-align: center;
        margin-bottom: 15px;
...
<section class="appointment" style="margin-top:120px;">
    ...
    <form method="POST" enctype="multipart/form-data" class="form">
        ...
        <!-- Upload New Photo -->
        <input type="file" name="profile_photo" accept="image/*" class="box">

        <!-- Full Name -->
        <input type="text" name="full_name" value="<?php echo $doctor['full_name']; ?>" placeholder="Full Name" class="box" required>

        <!-- Email -->
        <input type="email" name="email" value="<?php echo $doctor['email']; ?>" placeholder="Email" class="box" required>

        <!-- Phone -->
        <input type="tel" name="phone" value="<?php echo $doctor['phone']; ?>" placeholder="Phone" class="box" required>

        <!-- Specialty -->
        <input type="text" name="specialist" value="<?php echo $doctor['specialist']; ?>" placeholder="Specialty" class="box" required>
        ...
        <!-- Username -->
        <input type="text" name="username" value="<?php echo $doctor['username']; ?>" placeholder="Username" class="box" required>

        <!-- Password -->
        <input type="password" name="password" placeholder="New Password (leave blank to keep current)" class="box">

        <!-- Description -->
        <textarea name="profile_description" placeholder="Profile Description" class="box"><?php echo $doctor['profile_description']; ?></textarea>
